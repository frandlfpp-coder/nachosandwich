      ...
      <section>
        <div className="bg-card text-card-foreground rounded-2xl p-6 shadow-sm border">
          ...
          <div id="stock-grid" className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {stockItems.map(i => (
              <div key={i.id} className="flex items-center justify-between p-6 bg-secondary rounded-xl border">
                <div>
                  <h4 className="text-xs font-black">{i.name}</h4>
                  <span className="text-[8px] opacity-40 font-black">{i.unit}</span>
                </div>
                <div className="flex items-center bg-background rounded-xl border px-2 gap-4">
